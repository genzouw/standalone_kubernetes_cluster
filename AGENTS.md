...
- [ ] LLM プロバイダの API キー / 従量課金 API キーを GitHub Secrets に追加していない。`GEMINI_API_KEY` / `OPENAI_API_KEY` / `ANTHROPIC_API_KEY` 等を `secrets.*` から参照する記述が、新規ファイルだけでなく既存ファイルへの変更を含む本 PR の全差分に含まれていない。
...
1 章の MUST NOT から外れる導入を検討したい場合、PR を直接作成することは MUST NOT です。
本ポリシーから外れる導入を検討したい場合は、PR を作成する **前に** Issue で提案し、リポジトリオーナー (@genzouw) の明示的な承認を **MUST** 取得してください。承認された例外は、当該 Issue に明記された対象・範囲に限り、1 章の該当する MUST NOT 規定、および 2 章のチェックリストのうち当該例外に対応するチェック項目（例: 承認された新規 Secret 登録の場合、2 章の「LLM プロバイダの API キー / 従量課金 API キーを GitHub Secrets に追加していない」および「リポジトリオーナーへ新規 Secret の登録を依頼していない」の各項目）を上書きします。承認のない有料サービス導入 PR はクローズ対象とします。
Issue では「なぜ既存の無料サービスでは目的を達成できないか」「課金リスクをどう管理するか」「上書きの対象となる MUST NOT 項目」を明確に書いてください。
