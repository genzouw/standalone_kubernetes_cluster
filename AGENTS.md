...
本ポリシーから外れる導入を検討したい場合は、PR を作成する **前に** Issue で提案し、リポジトリオーナー (@genzouw) の明示的な承認を **MUST** 取得してください。承認された例外は、当該 Issue に明記された対象・範囲に限り、1 章の該当する MUST NOT 規定を上書きします。承認のない有料サービス導入 PR はクローズ対象とします。
